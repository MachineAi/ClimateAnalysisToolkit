...
namespace ClimateAnalysis {
    public partial class Ensembles : Form {
        ...
        public ProcessData.Ensemble[] getEnsembles() {
            if (ensembles == null && inputsValid())
                ensembles = createStandardEnsembles(low, middle, high, numPerEnsemble);
            return ensembles;
        }
        ...
        public void fillInDates() {
            ...
            List<ProcessData.DateRange> dates = datesForm.DateRange;
        ...
        private void okButton_Click(object sender, EventArgs e) {
            if (radioButton1.Checked && inputsValid())//use standard ensembles
                ensembles = createStandardEnsembles(low, middle, high, numPerEnsemble);
        ...
        private bool inputsValid()
        {
            ...
            low = getTextBoxFloat(txtStdLow);
            middle = getTextBoxFloat(txtStdMiddle);
            high = getTextBoxFloat(txtStdHigh);
            numPerEnsemble = getNumEnsembles(txtStdNumEnsembles);

            if (low < 0 || middle < 0 || high < 0 || numPerEnsemble < 0)
            {
                rval = false;
            }
        ...
        private float getTextBoxFloat(TextBox txtBox)
        {
            float rval = -1f;
            ...
            try
            {
                rval = float.Parse(txtBox.Text) / 100f;
        ...
        private int getNumEnsembles(TextBox txtBox)
        {
            int rval = -1;
        ...
        private void txtStdLow_TextChanged(object sender, EventArgs e) {
        ...
        private void txtStdMiddle_TextChanged(object sender, EventArgs e) {
            if (!thisFormIsVisible)
                return;
        ...
        private void txtStdHigh_TextChanged(object sender, EventArgs e) {
            if (!thisFormIsVisible)
                return;
        ...
        private void txtStdNumEnsembles_TextChanged(object sender, EventArgs e) {
            if (!thisFormIsVisible)
                return;
